...
## Shipment workflow coverage

Phase8 covers the local shipment workflow from imported orders to operational review, exception handling, CSV export, and results-center summary. The workflow remains local-first and uses the existing normalized order rows, shipment groups, product master, fare table data, and recommendation output.

## Shipment queue coverage

- 出荷キュー page exists for post-import shipment review.
- Queue rows include order count, issue count, recommended shipping method, current shipment status, and issue summary.
- Critical unresolved warnings are displayed as エラー.
- Blocking recommendation issues or missing recommendation output are displayed as 保留.
- Status actions update selected shipment-group orders without creating new storage keys.

## Shipment export coverage

- 出荷CSV出力 exports order-level shipment rows from current shipment groups.
- Default export excludes エラー / `error` and 保留 / `on_hold`.
- Export Preview shows CSV出力対象, 保留件数, エラー件数, and 配送会社別件数.
- Export rows include 出荷グループ, 注文番号, 顧客名, 郵便番号, 配送先住所, SKU, 数量, 推奨配送会社, 推奨サービス, and 出荷状態.

## Shipment results center coverage

- Results Center displays 総注文数, 出荷済み件数, 保留件数, and エラー件数.
- Carrier breakdown includes ヤマト, 佐川, and 日本郵便.
- Shipment status breakdown includes 取込済み, 確認待ち, 出荷準備中, 出荷済み, 保留, and エラー.
- Estimated savings indicators use existing fare recommendation output only: 推奨配送方式件数, 運賃比較対象件数, and 推定節約件数.
