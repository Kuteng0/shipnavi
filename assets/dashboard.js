...
function findMissingFields(headers, requiredFields) {
...
function getShipmentGroups() {
  ...
  const bundleableGroups = new Map();
  const standaloneGroups = [];

  orders.forEach((order) => {
    const product = getOrderProduct(order, products);
    const canBundle = product.bundleable === '可';
    ...
    if (!bundleableGroups.has(key)) bundleableGroups.set(key, []);
    bundleableGroups.get(key).push(order);
  });

  const groups = [...bundleableGroups.values(), ...standaloneGroups];
  return groups.map((ordersInGroup, index) => {
    const groupProducts = ordersInGroup.map((order) => getOrderProduct(order, products));
    const maxSize = Math.max(...ordersInGroup.map((order, orderIndex) => getOrderSize(order, groupProducts[orderIndex])));
    const firstOrder = ordersInGroup[0];
    return {
      shipmentGroupId: `SG-${String(index + 1).padStart(4, '0')}`,
      orders: ordersInGroup,
      customer: firstOrder.recipient,
      postal: firstOrder.postal,
      address: firstOrder.address,
      maxSize,
      zone: getOrderZone(firstOrder),
      bundled: ordersInGroup.length > 1,
    };
  });
}
...
function getBundleCandidates() {
  return getShipmentGroups()
    ...
    .map((group) => ({
      destination: `${group.customer} / ${group.postal} / ${group.address}`,
      orders: group.orders,
      shipmentGroupId: group.shipmentGroupId,
      saving: (group.orders.length - 1) * 320,
    }));
}

function matchesFareRow(row, shipmentGroup) {
  const rowSize = Number(String(row.size || '0').replace(/[^0-9]/g, '')) || 0;
  const rowZone = row.zone || row.region || '全国';
  return (!rowSize || rowSize >= shipmentGroup.maxSize) && (rowZone === '全国' || rowZone === shipmentGroup.zone);
}

function compareFaresForShipmentGroup(shipmentGroup) {
  const fareRows = getData('fareTables')
    .map(normalizeFare)
    .filter((row) => ['ヤマト運輸', 'ヤマト', '佐川急便', '佐川', '日本郵便'].some((name) => row.carrier.includes(name)));
  const carrierFallbacks = getData('carriers').map((carrier, index) => normalizeFare({
    carrier: carrier.name,
    service: carrier.service,
    zone: shipmentGroup.zone,
    size: shipmentGroup.maxSize,
    fare: 850 + (index * 80),
    fileName: 'fallback',
  }, index));
  const candidates = (fareRows.length ? fareRows : carrierFallbacks)
    .filter((row) => matchesFareRow(row, shipmentGroup))
    .map((row) => ({ ...row, fareNumber: Number(row.fare || row.price || 0) }))
    .filter((row) => row.fareNumber > 0)
    .sort((a, b) => a.fareNumber - b.fareNumber);
  const ranked = candidates.length ? candidates : carrierFallbacks
    .map((row) => ({ ...row, fareNumber: Number(row.fare || row.price || 0) }))
    .sort((a, b) => a.fareNumber - b.fareNumber);
  const lowest = ranked[0] || normalizeFare({ carrier: '未設定', service: '', fare: 0 });
  const second = ranked[1] || lowest;
  const lowestFare = Number(lowest.fareNumber || lowest.fare || lowest.price || 0);
  const secondFare = Number(second.fareNumber || second.fare || second.price || 0);

  return {
    shipmentGroup,
    shipmentGroupId: shipmentGroup.shipmentGroupId,
    recommendedCarrier: lowest.carrier,
    recommendedService: lowest.service || '',
    estimatedFare: lowestFare,
    lowestPrice: lowestFare,
    secondCarrier: ranked[1] ? second.carrier : '',
    secondService: ranked[1] ? second.service || '' : '',
    secondFare: ranked[1] ? secondFare : 0,
    secondLowestPrice: ranked[1] ? secondFare : 0,
    saving: ranked[1] ? Math.max(0, secondFare - lowestFare) : 0,
    candidates: ranked.map((row) => ({ carrier: row.carrier, service: row.service, fare: Number(row.fareNumber || row.fare || row.price || 0) })),
  };
}

function getFareComparisons() {
  return getShipmentGroups().map(compareFaresForShipmentGroup);
}

function getShipmentInstructions() {
  return getFareComparisons().flatMap((comparison) => comparison.shipmentGroup.orders.map((order) => ({
    shipmentGroupId: comparison.shipmentGroupId,
    ...
    service: comparison.recommendedService,
    estimatedFare: comparison.estimatedFare,
    secondCarrier: comparison.secondCarrier,
    secondFare: comparison.secondFare,
    savings: comparison.saving,
  })));
}
...
function renderProducts(filter = '') {
  ...
  `).join('') || '<tr><td colspan="9">商品がありません。</td></tr>';
}
...
function renderFareTables() {
  ...
  tbody.innerHTML = fares.map((fare) => `<tr><td>${escapeHtml(fare.carrier)}</td><td>${escapeHtml(fare.service)}</td><td>${escapeHtml(fare.size)}</td><td>${escapeHtml(fare.zone)}</td><td>${formatYen(fare.fare)}</td><td>${escapeHtml(fare.fileName)}</td></tr>`).join('') || '<tr><td colspan="6">運賃表がありません。</td></tr>';
...
function initOrders() {
  ...
  document.querySelector('#order-csv-form')?.addEventListener('submit', (event) => {
    ...
    readFileAsText(file, (text) => {
      const missingFields = findMissingFields(parseCsvHeaders(text), ['orderNo', 'customer', 'postal', 'address', 'sku', 'quantity']);
      if (missingFields.length) {
        showToast(`注文CSVに必須項目が不足しています: ${missingFields.join(', ')}`);
        return;
      }
...
function renderResults() {
  ...
  target.outerHTML = `
    <section class="stat-grid full-width"><article class="stat-card"><p>総注文数</p><strong>${instructions.length}</strong></article><article class="stat-card"><p>同梱候補数</p><strong>${bundleCandidates.length}</strong></article><article class="stat-card"><p>Shipment Group数</p><strong>${fareComparisons.length}</strong></article><article class="stat-card"><p>推定節約額</p><strong>${formatYen(getEstimatedSavings())}</strong></article></section>
    <section class="table-card full-width"><div class="table-toolbar"><h2>Mock 運賃比較結果</h2></div><div class="responsive-table"><table><thead><tr><th>Shipment Group</th><th>推薦配送会社</th><th>サービス</th><th>推定運賃</th><th>第二候補</th><th>第二運賃</th><th>節約額</th><th>全候補</th></tr></thead><tbody>${fareComparisons.map((item) => `<tr><td>${escapeHtml(item.shipmentGroupId)}</td><td>${escapeHtml(item.recommendedCarrier)}</td><td>${escapeHtml(item.recommendedService)}</td><td>${formatYen(item.estimatedFare)}</td><td>${escapeHtml(item.secondCarrier)}</td><td>${formatYen(item.secondFare)}</td><td><span class="badge green">${formatYen(item.saving)}</span></td><td>${item.candidates.map((candidate) => `${escapeHtml(candidate.carrier)} ${escapeHtml(candidate.service)}: ${formatYen(candidate.fare)}`).join('<br>')}</td></tr>`).join('') || '<tr><td colspan="8">比較対象の注文がありません。</td></tr>'}</tbody></table></div></section>
    <section class="table-card full-width"><div class="table-toolbar"><h2>出荷指示書プレビュー</h2></div><div class="responsive-table"><table><thead><tr><th>Shipment Group</th><th>注文番号</th><th>顧客</th><th>SKU</th><th>数量</th><th>推薦配送</th></tr></thead><tbody>${instructions.map((item) => `<tr><td>${escapeHtml(item.shipmentGroupId)}</td><td>${escapeHtml(item.orderNo)}</td><td>${escapeHtml(item.customer)}</td><td>${escapeHtml(item.sku)}</td><td>${escapeHtml(item.quantity)}</td><td>${escapeHtml(item.recommendedCarrier)} ${escapeHtml(item.service)}</td></tr>`).join('') || '<tr><td colspan="6">出荷指示がありません。</td></tr>'}</tbody></table></div></section>
  ...
  document.querySelector('#export-instructions-csv')?.addEventListener('click', () => {
    downloadCsv('shipnavi-shipment-instructions.csv', instructions, ['shipmentGroupId', 'orderNo', 'customer', 'postal', 'address', 'sku', 'quantity', 'recommendedCarrier', 'service', 'estimatedFare', 'secondCarrier', 'secondFare', 'savings']);
  });
}
