...
function normalizeProduct(product) {
  return {
    ...
    weight: product.weight || product['重量'] || '0',
    length: product.length || product['長'] || product['長さ'] || '0',
    width: product.width || product['幅'] || '0',
    height: product.height || product['高'] || product['高さ'] || '0',
    bundleable: product.bundleable || product['同梱可否'] || product['同梱'] || '可',
  };
}
...
function normalizeOrder(order) {
  return {
    ...
    orderNo: order.orderNo || order['注文番号'] || order['订单号'] || '',
    recipient: order.recipient || order.customer || order['受取人'] || order['顧客名'] || '',
    postal: order.postal || order['郵便番号'] || order['郵便番号'] || '',
    address: order.address || order['住所'] || order['住所'] || '',
    sku: order.sku || order.SKU || order['商品SKU'] || '',
    quantity: order.quantity || order['数量'] || '1',
...
function normalizeFare(row, index = 0) {
  ...
  return {
    id: row.id || makeId('f'),
    carrier: row.carrier || row['配送会社'] || row['运输商'] || carriers[index % Math.max(carriers.length, 1)]?.name || '未設定',
    region: row.region || row['地区'] || row.zone || '全国',
    size: row.size || row['尺寸'] || row['サイズ'] || '60',
    weight: row.weight || row['重量'] || '2kgまで',
    price: row.price || row['価格'] || row['价格'] || row.fare || '0',
